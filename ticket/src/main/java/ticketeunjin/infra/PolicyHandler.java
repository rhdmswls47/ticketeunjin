package ticketeunjin.infra;

import com.fasterxml.jackson.databind.DeserializationFeature;
import com.fasterxml.jackson.databind.ObjectMapper;
import javax.naming.NameParser;
import javax.naming.NameParser;
import javax.transaction.Transactional;
import org.springframework.beans.factory.annotation.Autowired;
import org.springframework.cloud.stream.annotation.StreamListener;
import org.springframework.messaging.handler.annotation.Payload;
import org.springframework.stereotype.Service;
import ticketeunjin.config.kafka.KafkaProcessor;
import ticketeunjin.domain.*;

//<<< Clean Arch / Inbound Adaptor
@Service
@Transactional
public class PolicyHandler {

    @Autowired
    TicketRepository ticketRepository;

    @StreamListener(KafkaProcessor.INPUT)
    public void whatever(@Payload String eventString) {}

    @StreamListener(
        value = KafkaProcessor.INPUT,
        condition = "headers['type']=='TicketReserved'"
    )
    public void wheneverTicketReserved_DecreaseTicket(
        @Payload TicketReserved ticketReserved
    ) {
        TicketReserved event = ticketReserved;
        System.out.println(
            "\n\n##### listener DecreaseTicket : " + ticketReserved + "\n\n"
        );

        // Sample Logic //
        Ticket.decreaseTicket(event);
    }
}
//>>> Clean Arch / Inbound Adaptor
